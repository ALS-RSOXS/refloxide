...
- `plugin` reflectivity helpers often assume a `(n_q, 2, 2)` polarization reflectivity block; confirm the rank returned by `tjf4x4` matches before applying matrix-style indexing; the `tjf4x4` kernel returns `[:, 0, 0] = R_pp` and `[:, 1, 1] = R_ss` (map p to `[:, 0, 0]`, s to `[:, 1, 1]`).
