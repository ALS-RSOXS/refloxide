...
- When replacing `pyref.fitting` call sites, keep the MaterialSLD/ReflectModel shape via `import refloxide.python.model as py`; name example helpers `py_*` (not `pyref_*`); do not rewrite examples to `materialize_at` plus raw `python.tmm` kernel calls.
...
- When calling the Rust reflectivity kernel from MCMC/fitting loops, pass `parallel=False` and parallelize walkers, chains, or multi-energy evaluations at the fitter/`Objective` level to avoid nested Rayon oversubscription.
...
- Prefer canonical public names (`RefloxideScatterer`, `DispersiveStructure`, `OocUniTensorScatterer`, `FreeTensorSLD`) over `EnergyDependent*` / `PXR_*` prefixes when adding or renaming APIs.
...
- Kernel polarization layout is `[:, 0, 0] = R_pp` and `[:, 1, 1] = R_ss` (Fresnel-validated; matches Rust / `python.tmm` packing). `refloxide.model.Reflectivity` remaps to physical channels (`.s` = R_ss from `[:,1,1]`, `.p` = R_pp from `[:,0,0]`). Legacy `pxr.layout.reflectivity_for_pol` already reads those diagonals in the physically correct order (`pol='s'` → `[:,1,1]`, `pol='p'` → `[:,0,0]`).
...
- Multi-energy fitting (`refloxide.model` / `refloxide.objective`): `ReflectModel.corrections` (`refloxide.instrument.ExperimentCorrections`) shares `energy_offset`/`dq`/`q_offset` across energies and keeps `scale_s`/`scale_p`/`bkg`/`theta_offset_s`/`theta_offset_p` per energy; slab `thick`/`rough` are not duplicated per energy; `density`/`rotation` live on the scatterer (`slab.sld`), not on `Slab`. Share one `OpticalConstants` across materials and memoize per energy (`cache_at`). `Objective` groups by energy with one materialize and one kernel call per requested pol (avoid a second kernel when `theta_s≠theta_p`); with `model.parallel=False`, parallelize energies at the Objective level. `Objective(nc_constraint=True)` enforces Nevot-Croce on `nll`/`residuals`/`logp_extra`. Force `POLARS_VERBOSE=0` before parquet IO when the environment exports verbose Polars logging. DE/`CurveFitter` paths should accept `workers=N`. Cache `CompiledReflectivityModel.parameters` (invalidate after structure edits); consume refloxide in refl-analysis via `uv sync --reinstall-package refloxide` then restart the Jupyter kernel; `refnx` `CurveFitter(workers=-1)` fails in notebooks (use an explicit worker count) and `Interval` bounds are not tuple-unpackable.
